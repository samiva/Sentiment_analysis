...
# tw = Twitter()
# df = pd.read_csv('./data/FinlandUsersID.csv', header=None)
# df = df.applymap(str)
...
names = ['Finland#liigaUsersID','Finland#VeikkausliigaUsersID','Sweden#AllsvenskanUsersID','Sweden#SHLUsersID']
for n in names:
    df = pd.read_csv('./data/' + n, header=None)
    df = df.applymap(str)
    x = df.to_string(header=False,
                    index=False,
                    index_names=False).split('\n')
